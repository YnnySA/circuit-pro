...
- Trazos simplificados: sin scatter de cuadrícula, menos annotations
- @st.cache_data en cálculos de onda para evitar recomputo innecesario
...
_C = {
    "R":   "#e63946",   # rojo — resistencia
    "X":   "#457b9d",   # azul — reactancia
    "XL":  "#6d28d9",   # violeta — inductiva
    "XC":  "#059669",   # verde — capacitiva
    "Z":   "#1d3557",   # azul oscuro — impedancia
    "phi": "#f4a261",   # naranja — ángulo
    "P":   "#e63946",
    "Q":   "#6d28d9",
    "S":   "#1d3557",
    "sin": "#1d6aa5",
...
def _sim_rlc():
    ...
    st.caption(
        ...
        "el ángulo de fase y el diagrama fasorial."
    )

    c1, c2 = st.columns([1, 1])
    with c1:
        V  = st.slider("Tensión V (V)",        1.0,  220.0, 100.0, 1.0,  key="rlc_V")
        R  = st.slider("Resistencia R (Ω)",    1.0,  500.0, 100.0, 1.0,  key="rlc_R")
        L  = st.slider("Inductancia L (mH)",    1.0,  500.0, 100.0, 1.0,  key="rlc_L")
        C  = st.slider("Capacitancia C (μF)",   1.0,  500.0, 100.0, 1.0,  key="rlc_C")
        f  = st.slider("Frecuencia f (Hz)",      1.0,  500.0,  50.0, 1.0,  key="rlc_f")
        mostrar_construccion = st.checkbox(“Mostrar líneas de construcción”, value=True, key="rlc_constr")
    ...
    with c2:
        ...
        if abs(X) < 0.5:
            st.success(f"🎯 **Resonancia** — f₀ = {f0:.1f} Hz", icon="⚡")
        elif X > 0:
            st.info(f"🔵 Inductivo (φ = {phi:.1f}°) | f₀ = {f0:.1f} Hz")
        else:
            st.info(f"🟢 Capacitivo (φ = {phi:.1f}°) | f₀ = {f0:.1f} Hz")

    # Diagrama fasorial — solo vectores esenciales, sin scatter de cuadrícula
    fig = go.Figure()
    margin = Z * 0.35
    ...
    fig.add_annotation(ax=0, ay=0, x=R, y=0, xref="x", yref="y", axref="x", ayref="y",
                       ...
                       text=f"R={R:.0f}Ω", font=dict(color=_C["R"], size=11))
    ...
    if abs(X) > 0.1:
        ...
        fig.add_annotation(ax=R, ay=0, x=R, y=X, xref="x", yref="y", axref="x", ayref="y",
                           ...
                           text=f"X={X:+.1f}Ω", font=dict(color=x_col, size=11))
    ...
    fig.add_annotation(ax=0, ay=0, x=R, y=X, xref="x", yref="y", axref="x", ayref="y",
                       ...
                       text=f"Z={Z:.2f}Ω", font=dict(color=_C["Z"], size=12, family="Segoe UI bold"))

    # Arco φ
    if abs(phi) > 1:
        t_arc = np.linspace(0, np.radians(phi), 30)   # 30 puntos (antes 40)
        r_arc = Z * 0.25
...
def _sim_potencias():
    ...
    st.caption(
        ...
        "el triángulo de potencias y el factor de potencia."
    )
    ...
    with c2:
        ...
        estado_fp = "✅ Bueno" if fp > 0.9 else ("⚠️ Mejorable" if fp > 0.7 else "❌ Bajo")
    ...
    # Triángulo de potencias — solo 3 vectores + arco
    fig = go.Figure()
    m   = S * 0.3
    ...
    fig.add_annotation(ax=0, ay=0, x=P, y=Q, xref="x", yref="y", axref="x", ayref="y",
                       ...
                       text=f"S={S:.3f} kVA", font=dict(color=_C["S"], size=12, family="Segoe UI bold"))
    ...
    fig.update_layout(
        **_LAYOUT,
        title=dict(text="Triángulo de Potencias", font=dict(size=13)),
...
def _calcular_onda(Am, f, phi_deg, dos_ciclos):
    """Cálculo cacheado de la onda — solo se recomputa si cambian los parámetros."""
    T      = 1 / f
    ciclos = 2 if dos_ciclos else 1
    t      = np.linspace(0, ciclos * T, 300)   # 300 puntos (antes 800)
...
def _sim_onda():
    ...
    with c1:
        Am  = st.slider("Amplitud máxima Vm (V)",  10.0, 400.0, 141.4, 0.5,   key="onda_Am")
        f   = st.slider("Frecuencia f (Hz)",         1.0,  200.0,  50.0, 1.0,   key="onda_f")
        phi = st.slider("Fase inicial φ (°)",       -180.0, 180.0,   0.0, 5.0, key="onda_phi")
        st.markdown("---")
        mostrar_rms     = st.checkbox("Mostrar línea V_rms",   value=True,  key="onda_rms")
        mostrar_periodo = st.checkbox("Mostrar período T",      value=True,  key="onda_T")
    ...
    with c2:
        ...
        fig.update_layout(
            **_LAYOUT,
            title=dict(text="Señal de Tensión Alterna", font=dict(size=13)),
            xaxis=dict(title="Tiempo (ms)", gridcolor=_C["grid"], zeroline=False),
            yaxis=dict(title="Tensión (V)", gridcolor=_C["grid"], zeroline=False,
